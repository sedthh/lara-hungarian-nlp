...
def commands():
	return {
		...
		"next"				: [{"stem":"next"},{"stem":"tovább"},{"stem":"előre"},{"stem":"még","wordclass":"regex","without":[{"stem":"ne"},{"stem":"egy"},{"stem":"1"}]},{"stem":"more"},{"stem":"continue"},{"stem":"folytat","wordclass":"verb"},{"stem":"folyta[st]+([ao]?d|ni|ás)?","wordclass":"regex"}],
		...
		"mute"				: [{"stem":"n[eé]m[aáií]\w{0,3}","wordclass":"regex","without":[{"stem":"vége"},{"stem":"vissza"},{"stem":"feloldás","affix":["a"]}]},{"stem":"mute","wordclass":"verb","prefix":[]}],
...
def smalltalk():
	return {
		...
		"about_skills"		: [{"stem":"mi(lyen|(ke)?t|k?re)\s(dolgok(at|ra)\s|tr[uü]k+[oö]k(et|re)\s|parancsok(at|ra)\s)?(tud(sz|n[aá]l)\s(csin[aá]lni|mutatni)?|ismersz|vagy\s(k[eé]pes|(be|meg)?tan[ií]tva)|tan[ií]tottak\s(be|neked)?|(k[eé]pes+[eé]gei?d|tulajdons[aá]g(o|ai)d)\svan(nak)?)","wordclass":"regex"}],
...
def cocktail():
	return {
		...
		"weather_rain"		: [{"stem":"(sz[uü]ks[eé]g(em)?|(fog\s)?kell(eni)?(\sfog)?|vigyek(\smagammal)?)(\slesz|\svan)?(\segy)?\s(es)?erny[oöő](t|re)?","wordclass":"regex"},{"stem":"(fog(\sma\s)?esni|esik\sma|esni\sfog(\sma)?)(\s(az\s)?es[oöő])?","wordclass":"regex","without":[{"stem":"hó"}]}],
		...
		"weather_sunny"		: [{"stem":"(s[uü]t(ni)?\s(fog\s)?(ma\s)?(a\s)?nap|meleg\s(id[oöő]\s)?(lesz(\sma)?|van(\smost)?(\s(oda)?kin+t?)?))","wordclass":"regex"}],
