...
def common():
	return {
		...
		"nvm"				: [{"stem":"felejtsd el"},{"stem":"mindegy","without":[{"stem":"hogy"},{"stem":"nem"}]},{"stem":"nem fontos"},{"stem":"hagy(jad?|d)","wordclass":"regex","with":[{"stem":"jól","affix":["van"]},{"stem":"á"},{"stem":"mindegy"},{"stem":"inkább"}]},{"stem":"ne\s(is\s)?(foglalkoz+(on|[aá]l)?|t[oö]r[oöő]dj([oö]n|[eé]l)?)\s(vel(e|[uü]k)|[ae][vz]+[ae]l)","wordclass":"regex"}],
...
def commands():
	return {
		...
		"exit"				: [{"stem":"(exit|quit)(elj([eé][dln])?)?","wordclass":"regex"},{"stem":"esc","affix":["ape"]},{"stem":"kilép","wordclass":"verb","prefix":[],"affix":["ás"]},{"stem":"bezár","wordclass":"verb","prefix":[],"affix":["ás"]},{"stem":"(l[eé]pj?([eé][dln])?.+?ki|z[aá]r(ja)?d?.+?be)","wordclass":"regex"},{"stem":"kapcsol\w+\s?(m[aá]r\s)?(ki|le)","wordclass":"regex"}],
		...
		"error"				: [{"stem":"error","wordclass":"noun"},{"stem":"hiba","wordclass":"noun"},{"stem":"rossz","wordclass":"adjective","without":[{"stem":"[eé]rzem|kedv(em)?|vagyok","wordclass":"regex"}]},{"stem":"nem (siker[uü]lt|j[oó]l?|m[uüű]k[oö]d(ik|[oö]tt)|ment)(\s\w)?(\s\w)?$","wordclass":"regex"},{"stem":"lefagyott"}],
...
def smalltalk():
	return {
		...
		"can_you_understand_me":[{"stem":"(meg)?[eé]rt(e(d|sz)|i)\,?((\shogy)?\sa?mit\s([ií]r|mond)\w+|\smagyarul)","wordclass":"regex"}]
	}
...
def cocktail():
	return {
		...
		"random_dice"		: [{"stem":"(dob[oó])?kock[aá]\w*","wordclass":"regex","with":[{"stem":"(dob[dj]|gur[ií]ts)([aáo][dln]?)?","wordclass":"regex"},{"stem":"(\d+|oldal[auú]s?)","wordclass":"regex"}],"without":[{"stem":"([eé]rm[eé]|p[eé]nz|fej)\w*","wordclass":"regex"}]},{"stem":"(dob[dj]|gur[ií]ts)([aáo][dln]?)?\.+?(\d+|hat\s?o\w+)","wordclass":"regex","without":[{"stem":"([eé]rm[eé]|p[eé]nz|fej)\w*","wordclass":"regex"}]}],
		...
		"calendar_add"		: [{"stem":"(megbesz[eé]l[eé]st?|me+ting\w*|id[oőö]pont(ok)?|napt[aá]r|[kc]al+end[aá]r|tal[aá]lkoz[oó]|teend[oöő]|esem[eé]ny)\w*","wordclass":"regex","with":[{"stem":"([ií]r[dj]([aáo][dln]?)?\s(be|fel|meg)|ve(d+|gy[eé][dln]?)\sfel|ad(j[aáo])?[dln]?|hozz([aáo][ldn]?)?\sl[eé]tre)","wordclass":"regex"},{"stem":"(felv[eé][stv]|hozz[aá]\s?ad|[ií]r[aá]s|foglal)\w*","wordclass":"regex"}]}],
		"calendar_remove"	: [{"stem":"(megbesz[eé]l[eé]st?|me+ting\w*|id[oőö]pont(ok)?|napt[aá]r|[kc]al+end[aá]r|tal[aá]lkoz[oó]|teend[oöő]|esem[eé]ny)\w*","wordclass":"regex","with":[{"stem":"(t[oö]r[oö]l[jd]([eéoö][dln]?)?|ve(d+|gy[eé][dln]?)\s(ki|le)|\w*t[oö]rl[eé]s\w*)","wordclass":"regex"}]}],
		"calendar_modify"	: [{"stem":"(megbesz[eé]l[eé]st?|me+ting\w*|id[oőö]pont(ok)?|napt[aá]r|[kc]al+end[aá]r|tal[aá]lkoz[oó]|teend[oöő]|esem[eé]ny)\w*","wordclass":"regex","with":[{"stem":"([ií]r[dj]([aáo][dln]?)?\s[aá]t|te(gye)?d*\s[aá]t|szerkes+z\w+|m[oó]dos[ií][ct]\w+|rak(j[aáo])?[ldn]?\s[aá]t)|[aá]t[ií]r[aá]s\w*","wordclass":"regex"}]}],
