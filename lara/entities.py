...
def common():
	return {
		...
		"hi" 				: [{"stem":"ha?i+","wordclass":"regex"},{"stem":"szia","affix":["sztok"]},{"stem":"helló","affix":["ka"]},{"stem":"szer[bv]usz(tok)?","wordclass":"regex"},{"stem":"szevasz"},{"stem":"hali","affix":["hó"]},{"stem":"j[oó]\s?(reggelt|napot|est[eé]t)","wordclass":"regex"}],
		"bye" 				: [{"stem":"bye"},{"stem":"viszlát"},{"stem":"viszont látásra"},{"stem":"jó éj","affix":["t","szakát"]},{"stem":"jóéjt"},{"stem":"jóccakát"},{"stem":"mennem kell"},{"stem":"csumi"},{"stem":"cs[aáoöő]+[oó]*(v[aá]z?)?","wordclass":"regex"}],
...
def smalltalk():
	return {
		...
		"user_friend"		: [{"stem":"(lesz(el|[uü]nk)|legy[uü]nk|lenn[eé]l|lehet([uü]nk|n[eé]nk))\s(az?\s)?(egyik\s|legjobb\s|k[eé]pzele?t(beli)?\s)?(bar[aá]to[km]|haverok|spano[km])","wordclass":"regex"},{"stem":"(bar[aá]to[km]|havero[km])\svagy(unk)?","wordclass":"regex"},{"stem":"te\svagy\sa.+?bar[aá]tom","wordclass":"regex"}],
		"how_are_you"		: [{"stem":"hogy vagy"},{"stem":"jól vagy"},{"stem":"(j[oó]l|hogy)\s[eé]rzed\smagad(at)?","wordclass":"regex"},{"stem":"mizu","affix":["js"]},{"stem":"hogy ityeg"},{"stem":"(hogy\stelt\sa|milyen(\svolt\sa)?)\snapod(\svan)?","wordclass":"regex"},{"stem":"w+h*a+[sz]+u+p+","wordclass":"regex"},{"stem":"(j[oó]|milyen)\s(a\s)?kedved(\svan)?","wordclass":"regex"},{"stem":"mi a(z [aá]bra| st[aá]jsz)","wordclass":"regex"}],
		...
		"about_you"			: [{"stem":"(mes[eé]lj|besz[eé]lj|mondj)([eo]n)?.+?mag(ad|[aá])r[oó]l","wordclass":"regex"},{"stem":"mutatkoz+([aá]l|on)?\s+be","wordclass":"regex"},{"stem":"(be)?muta(tn[aá]d|sd)\s.+?magad(at)?","wordclass":"regex"},{"stem":"([km]i(\s|\sa\s.+?)vagy te|te [km]i(\s|\sa\s.+?)vagy)","wordclass":"regex"}],
		"about_creator"		: [{"stem":"(ki|hogy(an)?)\s(a\s)?(k[eé]sz([ií]t([oöő]d|ett(ek)?)|[uü]lt([eé]l)?)|gazd[aá]d|programoz([oó]d|ott|tak)|[ií]rt[aá]k?|(hoz(ott|tak)|j[oö]tt[eé]l).+?(l[eé]tre|vil[aá]gra|k[oó]dod(at)?)|alkot([oó][dt]+|tak)|teremt(ett|[oöő]d)|(keresztelt|nevezett|adtak)\sel|adott\s(neked\s)?nevet)","wordclass":"regex"}],
		...
		"about_skills"		: [{"stem":"mi(lyen|(ke)?t|k?re) (dolgok(at|ra) |tr[uü]k+[oö]k(et|re) ) (tud(sz|n[aá]l) (csin[aá]lni)?|vagy (k[eé]pes|(be|meg)?tan[ií]tva)|tan[ií]tottak be|(k[eé]pes+[eé]gei?d|tulajdons[aá]g(o|ai)d) van(nak)?)","wordclass":"regex"}],
		...
		"can_you_learn"		: [{"stem":"(k[eé]pes( vagy)?|tud(sz)?) tanulni","wordclass":"regex"},{"stem":"tanulsz (is|a.+?b[oóöő]l)","wordclass":"regex"},{"stem":"[dln][aáeéo][km] (meg)?tan[ií]tani\b","wordclass":"regex","boundary":False}],
