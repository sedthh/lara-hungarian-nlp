...
def smalltalk():
	return {
		...
		"you_are_robot"		: [{"stem":"(te\s)?(egy\s)?(igazi\s|val[oó](s|di)\s|h[uú]s[\-\s]?v[eé]r\s)?(ember|szem[eé]ly|android)\svagy","wordclass":"regex"},{"stem":"(robot|chatbot|ai|mesters[eé]ges\sintel+igencia|g[eé]p|humanoid|programozva)\svagy","wordclass":"regex"},{"stem":"(emberrel|szem[eé]l+yel|robottal|programmal|algoritmussal|g[eé]ppel)\s(besz[eé]l(get)?ek|csevegek|levelezek|konzult[aá]lok)","wordclass":"regex"},{"stem":"(embernek|szem[eé]lynek|robotnak|programnak|algoritmusnak)\s([ií]ro(gato)?[km]|magyar[aá]zo[km])","wordclass":"regex"},{"stem":"(embernek|intelligensnek|szem[eé]lynek|robotnak|g[eé]pnek)\s(hiszed|tartod|gondolod)\smagad(at)?","wordclass":"regex"}]
	}
