...
def common():
	return {
		...
		"welldone"			: [{"stem":"fasza"},{"stem":"nagyszerű"},{"stem":"remek"},{"stem":"jó","prefix":["kurva"],"without":[{"stem":"nincs"},{"stem":"nem"},{"stem":"éjt"},{"stem":"reggelt"},{"stem":"napot"},{"stem":"estét"},{"stem":"éjszakát"}]},{"stem":"j[oó]l\s?van","wordclass":"regex"},{"stem":"király"},{"stem":"ügyes"},{"stem":"(sz[eé]p\s(volt|munka))|(ez\s(lesz\s)?az)|(sz?uper)","wordclass":"regex"},{"stem":"👍","wordclass":"emoji"},{"stem":"\(Y\)","wordclass":"regex","boundary":False},{"stem":"profi vagy"}],
	}
...
def commands():
	return {
		...
		"next"				: [{"stem":"next"},{"stem":"tovább"},{"stem":"előre"},{"stem":"még","wordclass":"regex","without":[{"stem":"ne"},{"stem":"egy"},{"stem":"1"}]},{"stem":"more"},{"stem":"continue"},{"stem":"folytat","wordclass":"verb"},{"stem":"folyta[st]+([ao]?d|ni|ás)?","wordclass":"regex"},{"stem":"következő","affix":["t"]}],
...
def cocktail():
	return {
		...
		"timer_add"			: [{"stem":"([aá]l+[ií](ts|c+s+)([aá]?[ld])?|csin[aá]lj([aá]l)?|rakj([aá]l)?|hozz\sl[eé]tre|vegy[eé]l\sfel)(\sbe)?(\segy|\saz?)\s(id[oöő]z[ií]t([oöő]|[eé]s)t|riaszt([aá]s|[oó])t|cs[oöe]ng([oöő]|et[eé]s)t|alarmo?t|[eé]breszt\w+)","wordclass":"regex"},{"stem":"sz[oó]l?j+([aá]l)?\s?(r[aá]m\s|nekem\s)?.+?(m[uú]lva|bel[uü]l)","wordclass":"regex"},{"stem":"kell\.+?kel[nj][ei]\w*","wordclass":"regex","without":[{"stem":"mikor"},{"stem":"hánykor"}]}],
