...
def cocktail():
	return {
		...
		"random_dice"		: [{"stem":"(dob[dj]|gur[ií]ts)([aáo][dln])?(\sf?el)?(\snekem)?(\saz?|\segy(et)?(\saz?)?)?\s([\w\d]+[\-\s]?(\w*\s)?)?(kock[aá](t|val))?","wordclass":"regex","without":[{"stem":"([eé]rm[eé]|p[eé]nz|fej)\w*","wordclass":"regex"}]}],
		...
		"shop"				: [{"stem":"(meg\s?)?((hol|ho(l|nnan)\studok|ho(l|nnan)\slehet|szeretn[eé][km]|akaro[km])\s?)?(ve(nni|hetn?[eé][km])|v[aá]s[aá]rol(ni|hatn?[aáo][km]))(\smeg)?(\sszeretn[eé][km]|akaro[km])","wordclass":"regex"}],
