...
def common():
	return {
		...
		"no"				: [{"stem":"nem"},{"stem":"ne"},{"stem":"soha"},{"stem":"mégse","affix":["m"]},{"stem":"rossz\s(v[aá]lasz|vicc|megold[aá]s)","wordclass":"regex"},{"stem":"nincs rendben"}],
		"hi" 				: [{"stem":"ha?i+","wordclass":"regex"},{"stem":"s+z+i+[aoó](ka|sztok)?","wordclass":"regex"},{"stem":"helló","affix":["ka"]},{"stem":"szer?[bv][au]sz(tok)?","wordclass":"regex"},{"stem":"hali","affix":["hó"]},{"stem":"(sz[eé]p|j[oó])\s?(reggelt|napot|est[eé]t)","wordclass":"regex"},{"stem":"[uü]dv[oö]z[oö]?l(l?ek|[oö]m)","wordclass":"regex"}],
		...
		"thx"				: [{"stem":"(k[oö]s+z|k[oösz][oösz][oösz])(i(ke)?|[oö]n[oö]m|[oö]nj[uü]k|[eoö]net|csi|ent+y[uüű])?(\s?sz[eé]pen)?","wordclass":"regex"},{"stem":"[ht][ht]x"},{"stem":"t(ha|h?e)nks?\s?(you)?","wordclass":"regex"}],
		...
		"command"			: [{"stem":"keres[eds]+n?","wordclass":"regex"},{"stem":"mutas[ados]+n?","wordclass":"regex"},{"stem":"mond[adjo]n?","wordclass":"regex"},{"stem":"néz[nz]?[eé]?[dl]","wordclass":"regex"},{"stem":"akaro[km]","wordclass":"regex"},{"stem":"utas[ií]t\w{1,}","wordclass":"regex"}],
		...
		"welldone"			: [{"stem":"fasza"},{"stem":"nagyszerű"},{"stem":"remek"},{"stem":"jó","prefix":["kurva"],"without":[{"stem":"nincs"},{"stem":"nem"},{"stem":"éjt"},{"stem":"reggelt"},{"stem":"napot"},{"stem":"estét"},{"stem":"éjszakát"}]},{"stem":"j[oó]l\s?van","wordclass":"regex"},{"stem":"király"},{"stem":"ügyes"},{"stem":"(sz[eé]p\s(volt|munka))|(ez\s(lesz\s)?az)|(sz?uper)|zs[ií]r","wordclass":"regex"},{"stem":"👍","wordclass":"emoji"},{"stem":"\(Y\)","wordclass":"regex","boundary":False},{"stem":"profi vagy"}],
	}

# menu commands
def commands():
	return {
		"ok"				: [{"stem":"ye","affix":["s","ah","p"]},{"stem":"igen"},{"stem":"aha"},{"stem":"ja","affix":["ja","h"]},{"stem":"ok","affix":["é","s","és","sa","ay","ézd","ézza"],"without":[{"stem":"nem"}]},{"stem":"úgy","without":[{"stem":"nem"}]},{"stem":"így","without":[{"stem":"(m[eé]g)?[ns]em?","wordclass":"regex"}]},{"stem":"jó","wordclass":"adjective","without":[{"stem":"(m[eé]g)?[ns]em?","wordclass":"regex"}]}],
		"cancel"			: [{"stem":"^([ae]z\s)?(\w+\s)?(nem?|no(pe|ne)?)(\s\w+)?(\s\w+)?$","without":[{"stem":"jó"},{"stem":"tud","wordclass":"verb"},{"stem":"sikerül","affix":["t"]}],"wordclass":"regex"},{"stem":"cancel"},{"stem":"mégse","affix":["m"]},{"stem":"elvetés"},{"stem":"ves[ds]e?\sel","wordclass":"regex"}],
		...
		"undo"				: [{"stem":"visszavon","wordclass":"verb","prefix":[]},{"stem":"vissza(.+?eg[eé]szet|l[eé]p([eé]s)?)","wordclass":"regex"},{"stem":"von.+?vis+za","wordclass":"regex"},{"stem":"undo"}],
		"redo"				: [{"stem":"mégis"},{"stem":"(meg)ism[eé]t(l[eé]s|el(je)?d?)?","wordclass":"regex"},{"stem":"el[oöő]rel[eé]p([eé]s)?","wordclass":"regex"},{"stem":"l[eé]p.+?el[oöő]re","wordclass":"regex"},{"stem":"redo"}],
		...
		"play"				: [{"stem":"(le)?j[aá](ts+z|c+)([aá]([dls]|ni))?(\sle)?(\svalamit?)?(\segy)?","wordclass":"regex"},{"stem":"play"},{"stem":"indít","wordclass":"verb","without":[{"stem":"újra"}]}],
		...
		"skip"				: [{"stem":"(kihagy\w+|hag+yj?a?d\ski|([aá]t|tov[aá]bb)(l[eé]p|ugr[aá])\w*|(ugor\w+|l[eé]p(je)?d)\s([aá]t|tov[aá]bb))","wordclass":"regex"},{"stem":"(sz?kip+(el\w*)?|m[aá]sikat)","wordclass":"regex"}],
		...
		"unmute"			: [{"stem":"n[eé]m[aáií]\w{0,3}","wordclass":"regex","with":[{"stem":"vége"},{"stem":"vissza"},{"stem":"feloldás","affix":["a"]}]},{"stem":"unmute","wordclass":"verb","prefix":[]}]
	}
...
def cocktail():
	return {
		...
		"random_dice"		: [{"stem":"(dob[dj]|gur[ií]ts)([aáo][dln])?(\sf?el)?(\snekem)?(\saz?|\segy(et)?(\saz?)?)?\s([\w\d]+[\-\s]?(\w*\s)?)?(kock[aá](t|val))?","wordclass":"regex"}],
		"random_card"		: [{"stem":"(h[uú]z+([aá]l)?|v[aá]las+z([aá]l)?)(\ski)?(\segy)?\s(lapot|k[aá]rty[aá](lapo)?t)","wordclass":"regex"}],
		"timer_add"			: [{"stem":"(id[oöő]z[ií]t([oöő]|[eé]s)t|riaszt([aá]s|[oó])t|cs[oöe]ng([oöő]|et[eé]s)t|alarmo?t|[eé]breszt\w+)","wordclass":"regex","with":[{"stem":"([aá]l+[ií](ts|c+s+)([aá]?[ld])?|csin[aá]lj([aá]l)?|rakj([aá]l)?|hozz\sl[eé]tre|vegy[eé]l\sfel)(\sbe)?","wordclass":"regex"}]},{"stem":"sz[oó]l?j+([aá]l)?\s?(r[aá]m\s|nekem\s)?.+?(m[uú]lva|bel[uü]l)","wordclass":"regex"},{"stem":"kell\.+?kel[nj][ei]\w*","wordclass":"regex","without":[{"stem":"mikor"},{"stem":"hánykor"}]}],
		"timer_remove"		: [{"stem":"(id[oöő]z[ií]t([oöő]|[eé]s)t|riaszt([aá]s|[oó])t|cs[oöe]ng([oöő]|et[eé]s)t|alarmo?t|[eé]breszt\w+)","wordclass":"regex","with":[{"stem":"([aá]l+[ií](ts|c+s+)([aá]?[ld])?\sle|kapcsol\w+\s(ki|le)|t[oö]r[oö]l\w+|off\w*)","wordclass":"regex"}]}],
		...
		"calendar_add"		: [{"stem":"(me+ting\w*|id[oőö]pont(ok)?|napt[aá]r|[kc]al+end[aá]r|tal[aá]lkoz[oó]|teend[oöő]|esem[eé]ny)\w*","wordclass":"regex","with":[{"stem":"([ií]r[dj]([aá][dl])?\s(be|fel|meg)|ve(d+|gy[eé][dl])\sfel|ad(j[aá])?[dl]|hozz([aá][ld])?\sl[eé]tre)","wordclass":"regex"},{"stem":"(felv[eé][stv]|hozz[aá]ad|[ií]r[aá]s|foglal)\w*","wordclass":"regex"}]}],
		"calendar_remove"	: [{"stem":"(me+ting\w*|id[oőö]pont(ok)?|napt[aá]r|[kc]al+end[aá]r|tal[aá]lkoz[oó]|teend[oöő]|esem[eé]ny)\w*","wordclass":"regex","with":[{"stem":"(t[oö]r[oö]l[jd]([eé][dl])?|ve(d+|gy[eé][dl])\s(ki|le)|\w*t[oö]rl[eé]s\w*)","wordclass":"regex"}]}],
		"calendar_modify"	: [{"stem":"(me+ting\w*|id[oőö]pont(ok)?|napt[aá]r|[kc]al+end[aá]r|tal[aá]lkoz[oó]|teend[oöő]|esem[eé]ny)\w*","wordclass":"regex","with":[{"stem":"([ií]r[dj]([aá][dl])?\s[aá]t|te(gye)?d+\s[aá]t|szerkes+z\w+|m[oó]dos[ií][ct]\w+|rak(j[aá])?[ld]\s[aá]t)|[aá]t[ií]r[aá]s\w*","wordclass":"regex"}]}],
		...
		"weather_rain"		: [{"stem":"(sz[uü]ks[eé]g(em)?|(fog\s)?kell(eni)?(\sfog)?|vigyek(\smagammal)?)(\slesz|\svan)?(\segy)?\s(es)?erny[oöő](t|re)?","wordclass":"regex"},{"stem":"(fog\s?(ma\s)?esni|esik\sma|esni\sfog(\sma)?)(\s(az\s)?es[oöő])?","wordclass":"regex","without":[{"stem":"hó"}]}],
		...
		"weather_sunny"		: [{"stem":"(s[uü]t(ni)?\s(fog\s)?(\w+\s)?(a\s)?nap|meleg\s(id[oöő]\s)?(lesz(\sma)?|van(\smost)?(\s(oda)?kin+t?)?))","wordclass":"regex"}],
		...
		"summary"			: [{"stem":"mik?\s(van(nak)?|lesz(nek)?|volt(ak)?|t[oö]rt[eé]nt(ek)?)(\sa)?\sm[aá](i(\snapon)?|ra)?","wordclass":"regex"},{"stem":"összefoglal","wordclass":"noun"},{"stem":"foglal\w+\s[oö]s+ze","wordclass":"regex"},{"stem":"mond(ja)?d?\sel\s(r[oö]viden|t[oö]m[oö]ren|egyszer[uüű]en|[oö]s+zefoglalva)","wordclass":"regex"}],
		...
		"shop"				: [{"stem":"(meg\s?)?((hol|ho(l|nnan)\studok|ho(l|nnan)\slehet|szeretn[eé][km]|akaro[km])\s?)?(ve(nni|hete[km])|v[aá]s[aá]rol(ni|hato[km]))(\smeg)?(\sszeretn[eé][km]|akaro[km])","wordclass":"regex"}],
