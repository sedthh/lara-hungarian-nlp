...
def _tippmix_stemmer_get_prefix(word):
	...
	if length <= 3:
		return (word,)
	prefixes = (
	"abba", "alá", "bele", "benn", "ellen", "elő", "fel", "föl", "hátra", "hozzá", "ide", "körül", "meg", "mellé",
	"neki", "oda", "össze", "szét", "túl", "utána", "vissza")
		...
		for word in word_list:
			if len(word) > 4:
				...
				if word[-1] in ('a', 'e'):
					...
					if len(word) > 4:
						if re.findall(r'(ameri[ck][aá]|eur[oó]p[aá]|eur[aá]zsi[aá]|afri[ck][aá]|[aá]zsi[aá])t', word,
									  re.IGNORECASE):
							word = word[:-1]
						else:
							if vh == 'magas':
