...
def strip_context(text, context="search", including=None):
	if text:
		if context=='search':
			...
			exclude		= re.compile(r'\b(szia|szervusz|[uü]dv([oö]zl(et(tel)?|[eoö]m))?|(j[oó])?(reggelt|napot|est[eé]t)|k[ií]v[aá]nok|szeret(n[eé][km]|t([uü]k|n[eé]nk|em))|(meg)k[eé]r(dez(ni)?|ni)|k[eé]rd[eé]s(t|em|ek|eim|sel|emmel|ekkel)?|[eé]rdekl[oöő]d(ni|[oö]k|n[eé]k)?|[eé]rdekel(ne)?|v[aá]r(o[mk]|unk|juk)?|tisztelt|kedves|v[aá]lasz(ol(ni|t|tak)?|uk(at)?|t)?|[eé]rdekl[oöő]d(ni|n[eé]k|[oö]m)|az([eé]rt)?|[ae]zz[ae]l|[ae]bb[ae]n|(a)?miatt|abb[oó]l|[io]lyan(kor)?|lehet|lenne|tenni|teend[oő](m|nk)?|mert|(a)?hogy(an)?|[uü]gyben|kapcsolat(ban|os|osan)|(meg)?tudn[aá](nak)?|tan[aá]cs((uk)?ra|(uk)?[aáo]t)|k[oö]sz(i|n[oö]m|nj[uü]k|nettel)?)\b', re.IGNORECASE)
			text		= exclude.sub('',text)
		if including:
