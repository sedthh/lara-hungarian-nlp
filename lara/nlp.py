...
def strip_context(text, context="search", including=None):
	if text:
		if context=='search':
			...
			exclude		= re.compile(r'\b([ae](z([eo]k)?a?t?|([eo]k)r+[ae])?|a?m?[ieo]ly[ae]\w+|a?mi(kor)?|a?hol|hogy|van(nak)?|[mk]i(vel|nek|[eé]rt|t|k(et)?)?|r[aá]|egy|meg|be|kell(ene)?|k[eé]ne|szeretn[eé][km]|k[eé]rn?([eé][km]|i)?|ad[dj]([aá][dl])?|nekem|van|csak|k[uü]ld[dj]?[eé]?[dl]?|mond\w*|[ae]bb[ae]n?|l[eé]gy([eé]l)?\s?sz[ií](ves)?|l[eé]cci|azt[aá]n|vagy|m[aá]sik(at)?|lesz|legyen|lenne|vala[km]i(lyen|t[oöő]?l?|nek|[kv]el|[eé]rt?)?|sz[uü]ks[eé]g(em)?|ink[aá]bb|akkor|volt|amiben|keres\w+|[eé]?s|[ae]rr[ae])\b', re.IGNORECASE)
			text		= exclude.sub('',text)
		elif context=='mail':
